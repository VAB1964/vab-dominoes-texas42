...
export interface GameView {phase:Phase;dealerSeat:number;turnSeat:number|null;bidderSeat:number|null;highBid:number|null;trump:Trump|null;ledSuit:Pip|null;trick:{seat:number;domino:Domino}[];hand:Domino[];widowCount:number;handNumber:number;message:string;winnerSeat:number|null;teamMarks:[number,number];teamHandPoints:[number,number];winnerTeam:number|null;}
